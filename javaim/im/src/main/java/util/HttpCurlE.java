...
public class HttpCurlE {
    ...
    public void tryCurl() throws IOException, HttpException {
        ...
        getImgCode(httpclient,imgCodeUrl,saveFileName,context);

        String cookieBeforeLogin = getCookie(cookieStore.getCookies());

        logger.info("===== cookie before login " + cookieBeforeLogin);
        ...
        httpGetIndexToken.setHeader("Cookie",cookieBeforeLogin);
        httpGetIndexToken.setHeader("Connection","keep-alive");

        HttpResponse responseTokenAndParam = httpGet(httpclient,httpGetIndexToken,context);

        String cookieAfterGetToken = getCookie(cookieStore.getCookies());

        logger.info("===== cookie after get token " + cookieAfterGetToken);
        ...
        HttpResponse responseData = httpGet(httpclient,getData,context);
        ...
        String cookieAfterLogin = loginForCookie(httpclient,loginUrl,token1,code,cookieAfterGetToken,context);

        logger.info("===== data " + data);
        logger.info("===== token1 " + token1 + " params " + params + " " + dataMap.toString() + " loginCookie " + cookieAfterLogin);
        ////get token getData params end

        //拿到token
        String refererUrl = planUrl + "?rq=" + dataMap.get("rq") + "&sjd=" + "&pbid=" + dataMap.get("pbid");

        logger.info("===== refererUrl " + refererUrl );
        HttpGet httpGetToken = new HttpGet(refererUrl);
        httpGetToken.setHeader("Cookie",cookieAfterLogin);
        httpGetToken.setHeader("Connection","keep-alive");
        HttpResponse responseToken2 = httpGet(httpclient,httpGetToken,context);
        ...
        List<Cookie> cookiesList4 = context.getCookieStore().getCookies();
        logger.info("===== " + token3 + " " + tokenHtml);
        ...
        HttpResponse responseCode2 = httpGet(httpclient,httpGetCode2,context);
        //get img and cookie
        List<Cookie> cookiesList = context.getCookieStore().getCookies();
        ...
        addCookies.process(planPost, context);
        //报名成功
        HttpResponse responsePlan = httpclient.execute(planPost,context);
        logger.info("===== " + responsePlan.getStatusLine().getStatusCode());
    ...
    public void getImgCode(CloseableHttpClient httpClient,String imgCodeUrl,String saveFileName,HttpClientContext httpClientContext) throws IOException {
        HttpGet httpGet = new HttpGet(imgCodeUrl);
        httpGet.setHeader("Connection","keep-alive");
        HttpResponse responseCode = httpGet(httpClient,httpGet,httpClientContext);
        //get img and cookie
        //保存img
    ...
    public String loginForCookie(CloseableHttpClient httpClient,String loginUrl,String token1,String code,String cookie1,HttpClientContext context) throws IOException {
        ...
        //登录成功
        HttpResponse responseLogin = httpClient.execute(loginPost,context);
    ...
    public HttpResponse httpGet(HttpClient client,HttpGet httpGet,HttpClientContext context){
        try {
            if(context != null){
    ...
    public String getCookie(List<Cookie> cookieList){
        ...
        for (int i = 0; i < cookieList.size(); i++) {
            cookiesSB.append(cookieList.get(i).getName()).append("=")
                    .append(cookieList.get(i).getValue());
        }
